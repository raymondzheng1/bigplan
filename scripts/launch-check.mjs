...
check(html.includes('respawnRepeat'), 'recurring tasks (rituals) respawn with streaks');
check(html.includes('queueName'), 'completions remember their context queue');
check(html.includes('renderTbChips'), 'timebox lives inside work mode');
check(read('api/cron/daily.js').includes('RITUALS'), 'daily email covers rituals + queue breakdown');
